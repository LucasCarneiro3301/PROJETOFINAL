...
void operating_mode(ssd1306_t *ssd, bool color, double voltage, double current, double u, double volt_mode) {
    ...
    (voltage <= (volt_mode+volt_mode*0.1) && (current <= 0.5+0.5*0.1)) ? ws2812('*') : ws2812('x'); // Alerta ao usuário caso a tensão ou corrente ultrapasse os limites permitidos
    (u >= 0.0) ? pwm_set_gpio_level(BLUE, (uint16_t)(u * (WRAP + 1))) : pwm_set_gpio_level(BLUE, 0); // Atualiza o duty cycle baseado no controle u (0 a 1)
    ...
    ssd1306_draw_string(ssd, str, 75, 10); // Mostra o nível da corrente do indutor atual
    ssd1306_draw_char(ssd, 'A', 110, 10);

    sprintf(str, "%.2f", (u >= 0) ? 100*u : 0.0);

    ssd1306_draw_string(ssd, "PWM:", 10, 30);  
    ssd1306_draw_string(ssd, str, 45, 30); // Mostra em porcentagem o ciclo de trabalho atual do PWM
    ssd1306_draw_char(ssd, '%', (u >= 0.1) ? 88 : 80, 30);

    sprintf(str, "%.2f", volt_mode);

    ssd1306_draw_string(ssd, "REF:", 10, 50);
    ssd1306_draw_string(ssd, str, 45, 50); // Mostra a tensão de referência
