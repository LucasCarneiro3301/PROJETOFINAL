...
void ssd1306_draw_char(ssd1306_t *ssd, char character, uint8_t x, uint8_t y) {
  uint16_t index = 0, row = 8, col = 8;

  if(character >= 'a' && character <= 'z') {index = (character - 'a' + 37) * 8; col = 8;} // Para letras minúsculas
  else if (character >= 'A' && character <= 'Z') index = (character - 'A' + 11) * 8; // Para letras maiúsculas
  else  if (character >= '0' && character <= '9') index = (character - '0' + 1) * 8; // Para números
  ...
  else  if (character == '.') index = (character - '.' + 66) * 8; // Para .
  else  if (character == '-') index = (character - '-' + 67) * 8; // Para -
  else  if (character == '%') index = (character - '%' + 68) * 8; // Para %
